...
- 改完跑 `npm run typecheck && npm run build` 就提交；取舍拿不准先问，被否的方向不换说法再来
...
- 回到表单：内容已成型停最后一步（`formAtLastStep`），否则从第一步进（`openForm`）；按钮四种变体只差颜色（外高 40px、内边距 32px、描边 1px），危险动作靠颜色与文案
...
- 档位只看宽度（数在 `@theme`，代码里无宽度数字）：窄 < 30rem（`max-narrow:`）、中（默认）、大 ≥ 64rem（`wide:`）；`landscape:` / `portrait:` 已退役
...
- 没有兜底文案：标题块连横线、页脚署名行都不渲染；条件渲染的间距用 `gap`；占位提示全站一句「不显示」；预填值不显示就不写
