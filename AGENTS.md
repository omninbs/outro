...
- **最终页（`OutroPage`）也跟这三档**，只是它是「拿去截图的那一屏」，有三处自己的排版细节：① 版面宽就是「**一行**的宽」——中档单栏 `max-w-[24rem]`，宽档 `wide:max-w-[48rem]` 正好一倍（宽档把那两栏并排，一栏仍是一行的宽，所以两种档位下行长是同一个数）；窄屏不另给数，它本来就比 24rem 窄，于是自然全宽 + `px-inset`，384–480 之间只差那两条内边距（16 / 24），行宽不变、不会「窗口变宽文字反而变窄」；② 整块**横竖都居中**（`justify-center-safe`，内容比屏幕高时退回顶部排，不会被切掉上半截）；③ 纵向那个「重心上移 28px」的光学补偿用**内边距**拿（`pb-26`，窄屏 `pb-22`：下面比上面多 56px），不用 `translate`——translate 在内容比屏幕高时会让标题只剩 4px 边距（探针量到过）。它身上没有「面」（无卡片 / 底色 / 描边），所以窄屏的边距内化落到这里就是 `px-6` 收到 `max-narrow:px-inset` 这一条；`wide:` 才分两栏（左元数据 1.2 : 右文本块 1）；④ 页脚与元数据行都**只在最窄那一档折叠**（`max-narrow:`）——中档那个 24rem 一行放得下，没必要提前把结构拆开
