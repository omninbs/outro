...
- **换行要么由档位定，要么由内容定，别让它悬着**：一行里放几样东西是**结构**，要按档位写死——页脚两端（`wide:flex-row` 才左右分列，否则上下两行）、动作行、两栏；`flex-wrap` 只留给**条数 / 长度由数据决定**的排（`QuestionInput` 的选项、`Stepper` 的步骤条）。`GenerateStep` 的按钮行是唯一的兜底（两个短按钮，防 320px 以下挤爆），不算「内容驱动布局」的先例。最终页页脚原本是 `flex-wrap justify-between`，改回档位就是因为那条判断没写下来——看的人不知道它什么时候会换行
