...
- **一维的排布用 flex**：一行或一列都写成 `flex` + `gap-N`（`flex-row` / `flex-col`），**一维的组合**（列里嵌行、行里嵌列）也一样，别因为嵌套就换工具。`space-y-*` 2026-09 全清掉了：它靠 `> :not(:last-child)` 的边距撑开，块不在了那份距离还在（跟 `mt-12` 一个毛病），子元素自己的边距也会跟它合并。`grid` 只留给**真二维、行列都要对齐**的地方（现在只有元数据那张 `dl`：名称列要跨行对齐），而且必须显式写列模板
