...
- 问卷是数据不是代码：题目写 `into` 决定答案落到结尾页哪里（四条去处写成一个字符串：`into: 'meta'` / `'block'` / `'title'` / `'footer'`；2026-09 从 `{ kind, label? }` 压扁的——可选的那层 `label` 11 道题一处都没写过，结尾页那行名字一律取题面），需要加工才写 `build`；加一份问卷不用碰组件
