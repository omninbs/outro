 ...
 *
 * ① 版面的宽按「一行的宽」定：中档 `max-w-[24rem]`，宽档 `wide:max-w-[57rem]`（两栏各 24rem），
 *    比向导的容器窄一档——行短了看着更紧，截图也更像一张版面；
	...
	return (
		<div class="flex flex-1 flex-col justify-center-safe">
			{/* 这一层就是版面本身：宽度上限 + 一圈内边距，四边同一个 48（`p-12`）。
			    宽度按「一行」的宽定：中档单栏 `max-w-[24rem]`；宽档 `wide:max-w-[57rem]`
			    = 两栏各 24rem（48rem）+ `gap-12`（3rem）+ 左右 `p-12`（6rem），所以宽档里
			    **每一栏也还是这一行的宽**，两种档位下行长同一个数（2026-09 用户定的：
			    「大屏保证两栏都有 24rem」——改之前容器是 48rem，一栏只有 312px）。
			    窄屏**不另给上限**（`max-w-[24rem]` 留着），只把内边距收到 `p-inset`（16）
			    ⇒ 内容最宽 352px；窗口在 384–480 之间（属窄档）时它是居中的 384px 柱子、
			    两侧留白，不是全宽——横竖留白仍是同一个数，跟全站同一条竖线。
			    （2026-09 之前这里是 `pt-12 pb-26`：下面多 56px，把整块重心顶高 28px 当光学补偿。
			    用户要求内边距取同一个值，那套补偿连同它的注释一起收掉了——别再悄悄加回来。） */}
			<div class="mx-auto w-full max-w-[24rem] p-12 wide:max-w-[57rem] max-narrow:p-inset">
