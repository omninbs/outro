...
/**
 * 视图状态：应用只有「填写」（wizard）和「版权页」（colophon）两个页面。
 *
 * 用 hash 记录当前视图，判断规则只有一条：地址里出现 `#colophon` 就是最终页，其余都是向导。
 * 用 hash 而不是路径，是因为改 hash 不触发导航，构建产物直接用浏览器打开（file://）时
 * 照样能用、能刷新、能前进后退；路径路由在 file:// 下会直接失败。
 */
export type View = 'wizard' | 'colophon';

const COLOPHON_HASH = 'colophon';

function readView(): View {
	const name = window.location.hash.slice(1).trim().toLowerCase();
	return name === COLOPHON_HASH ? 'colophon' : 'wizard';
}
...
function writeView(view: View) {
	...
	window.location.hash = view === 'colophon' ? COLOPHON_HASH : '';
}
...
export function RouterProvider({ children }: { children: ComponentChildren }) {
	// 首屏直接读 hash：带 `#colophon` 打开或刷新时就落在最终页。
