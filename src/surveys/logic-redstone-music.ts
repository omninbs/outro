 ...
 *   再是它的 id、题面、输入形态、预填值
 * - 「作品名称 / JE26.1+ / 单调（无分歧）」这类是**预填值**（`default`）：答题框里一开始就写着，
 ...
 * - 两道文本块题分两种：尾注内容留空（不写 `default`），指南的默认值**用选项给出**——
 *   `options` 里就那一条、`default` 指向它，所以一进问卷它是**选中状态**，不用先看见一个
 *   写满字的框（想写别的就点「自定义」；再点一下那条就是取消，不印这一段）。
 *   文本块按题目顺序自上而下排，所以结尾页最下面那条是「指南」，尾注排在它前面
 *   （原来还有一道「特别说明」，已删）
